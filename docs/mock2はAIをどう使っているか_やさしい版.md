...
2026-08-20 作成 / 2026-08-21 §10を追記。読む人: 作者(コードを読まない前提)。
...
## 10. 小さいAIを安定させるには(2026-08-21 追記)

作者の相談: 「いまGemma 4 E4Bで動かしているが、起動環境・遅さ・ハルシネーションに困っている。
E2B(もっと小さいAI)にQLoRAを当てれば安定しないか」。

その場でコードを読み直して答えた内容を残す。**Ollamaが起動していなかったため実測はできていない。**
数字の出どころは、この文書の他の節とコードのコメントである。

### 10-1. まず、3つの困りごとのうち直せるのは1つだけ

| 困りごと | 学習(QLoRA)で直るか |
|---|---|
| **遅い** | **直る。** ただし理屈は「AIが小さくなるから」ではなく**手紙が短くなるから** |
| **ハルシネーション** | **半分だけ。** 用紙の書き間違い・口調崩れは直る。**未開示の真相の捏造は直らない** |
| **起動環境** | **直らない。** むしろ増える。学習したAIをブラウザ用の形へ変換する工程が加わる |

3つ目が肝心なので先に書く。Ollamaの起動、モデルの読み込み、`keep_alive`が切れて次の手番が急に遅くなる——
これらは**AIの賢さとは無関係の配管の問題**で、学習では1つも直らない。

2つ目も正確に切り分けておきたい。§6のとおり、**進行の捏造はすでに封じてある**
(「次の場面へ」は出口を持つシーンでは常に却下、HPは-3〜+2に丸める、拾える品は`loot`にある物だけ)。
未開示の真相も、手紙にそもそも書いていない。だから**いま残っている不調のほとんどは§8の
「用紙の書き方の間違い」**である。ここは学習が得意な領域。

### 10-2. なぜ間違えるのか — 手紙が重いから

§3の実測どおり、語りAIへの手紙は**4,446文字**。そのうち「# ルール」の箇条書きが**14本**あり、
その多くが**禁止則**である(台詞を地の文に書くな、移動を語るな、通路を作るな、数値を出すな……)。

**「〜するな」を14本、同時に守らせる**のが小さいAIには重い。しかも1回の呼び出しで
「情景 + 同行者の一言 + NPCの台詞 + 判定の申し出 + 状態変更の提案」を全部同時に要求している。
禁止則が14本必要になっているのは、**1つの用紙に欄を詰め込みすぎている**結果でもある。

§8の経験則「欄を増やすと壊れやすくなる」の裏返しで、**欄を減らすと安定する**。

### 10-3. 安い順に3手。学習は3番目

**手1. 用紙の「形」をAIに強制させる(学習なし)**

いまローカルAIには「JSONで返せ」とだけ指示している(コードで確認済み: `format: "json"`)。
これは**カッコの対応が合っているか**だけの保証で、**欄の名前と中身の種類は保証されていない**。

Ollamaには「用紙そのもの」を渡す口がある。渡すと、
`companion.who`は**リディアかガレスのどちらかしか書けない**、`hp_delta`は**数字しか書けない**、
という制約をAIが文章を作る段階で守らされる。§8の`aside_true_false_足元を見て`のような
混ざり方は**原理的に起きなくなる**。

*代償*: 形を強制すると地の文の質が落ちるという報告がある(**未確認**)。
また実行時の版によっては、§4の「手紙の前半を使い回す」仕組み(31秒→3秒)が効かなくなることがある。
**やったら測り直しが必要。**

**手2. 1回の呼び出しを2つに割る(学習なし)**

「情景を書く」と「誰が何を言うか」を別の手紙にする。手紙が2通に分かれるぶん、
1通あたりの禁止則が減る。小さいAIでは**短い手紙2回のほうが、長い手紙1回に勝つのが定石**。

しかも`npm run test:playthrough:hybrid`(通しプレイの検査)があるので、
割る前と後を**同じ手番数で比べられる**。

*代償*: 往復が2回になるので、1手番の合計時間は伸びるかもしれない。これも測らないと分からない。

**手3. QLoRA(学習)**

「QLoRA」は、**毎回手紙に書いている決まりを、AIの中に覚え込ませる**こと。
覚えたぶんは手紙から削れるので、4,446文字が1,000文字級まで縮む見込みがある。
§4のとおり**手紙の長さが遅さの主因**なので、これが速度への一番効く手になる。文体・人格も安定する。

**ただし手1と手2の後にやること。** 先に学習すると、4,446文字の手紙を前提にした癖を覚えてしまい、
後で手紙を短くしたときに作り直しになる。

### 10-4. E2Bに落とすだけでは悪くなる

`src/llm.js`にはすでに`gemma-4-E2B-it-web`(ブラウザの中で動く小さいほう)が配線されている(§7)。
つまりE2Bは**もう試せる状態**。

しかし**E2B単独への降格は品質が落ちるだけ**である。禁止則14本を守るのは
E2B級には荷が重い。**QLoRAは「E2Bを選ぶための必須条件」であって、選択肢ではない。**
「E2Bにする」と「QLoRAを当てる」は**セットで初めて成立する**。

### 10-5. 先に潰すべきリスクは2つ

**(a) 学習データがまだ足りない**

学習には「手紙 → 正しい記入済み用紙」の対が大量に要る。
手元にあるのは`playlogs/`の7ファイル(2026-08-13、計約50KB)と、
プレイ記録(`chron`)。**数十手番ぶんで、必要量に桁が2つ足りない。**

現実的な作り方は**蒸留**——賢いAI(Claude / Gemini)で大量に手番を回し、
**作者が「これは良い」と認めた出力だけを残して**、それを小さいAIに教える。
`.env`のバックエンドを切り替えるだけで同じ配管が動くので、回すこと自体はすぐできる。

つまり**最初に作るべきは学習の仕組みではなく、「採用/却下を記録する仕組み」**である。
これはエディタの話とつながる。

**(b) 汎用性を殺す危険**

lanternhillのデータだけで学習させると、**世界の事実まで覚え込む**。
「マイラは村の顔役」「灯りの番人がいる」がAIの中に焼き付き、
**次の物語を作ったときに今より悪くなる。**

歯止め: **学習対象は「用紙の書き方」と「禁止則の遵守」に限り、世界の固有名詞を含む例を入れない。**
ただしキャンペーンが1本しか無い段階では、この歯止めが効いているかを**確かめる方法が無い**。

### 10-6. 判断が必要な1点(未決)

**このゲームを「lanternhill専用の完成品」にするか、「作者が別の物語も作れる道具」にするか。**

- **専用でよい** → QLoRAは強い手。世界の事実まで覚え込ませて構わない
- **道具にしたい** → 学習は用紙と禁止則だけに限る。そして**2本目のキャンペーンができるまで学習は待つほうがよい**
  (1本しか無いと、汎用性が壊れていないことを確かめられない)

コードからは決められない。作者の判断待ち。

### 10-7. 測るべき数字(まだ測っていない)

相談の時点でOllamaが起動しておらず、**実測はゼロ**。判断に要る数字は3つだけで、既存の仕組みで取れる。

E4Bそのまま / E2Bそのまま で、1手番あたり:

| 測るもの | 取り方 |
|---|---|
| **用紙の書き間違い率** | `parseLlmJson`が1回目で読めた割合(修復に回った回数) |
| **禁止則の違反率** | 地の文に「」が入った回数、`sanitizeSay`が実際に何か削った回数 |
| **前処理の秒数と生成の秒数** | `callOllama`がOllamaの実測値をすでに返している |

**この3つで「学習で埋まる差か」が決まる。** E2Bの違反率がE4Bの2倍程度なら埋まる見込みがあるが、
5倍10倍なら手数が足りない。
...
| ファインチューニング | AIそのものに覚えさせること。手紙に毎回書かずに済むようになる |
| QLoRA | ファインチューニングの安価な方式。AI全体ではなく差分だけを学習し、家庭のPCでも回る |
| 蒸留 | 賢いAIの出力を教材にして、小さいAIに真似させること |
| 構造化出力 | 「用紙の形」をAIに強制する仕組み。欄の名前と中身の種類まで守らせる |
| E2B / E4B | Gemma 4の大きさ違い。E2Bのほうが小さく速く、そのぶん決まりを守るのが苦手 |
...
| 内容 | ファイル |
|---|---|
| ローカルAIへの指示(`format: "json"`、温度0.2、文脈長16384) | `server.cjs` の `callOllama()` |
| 壊れたJSONを4通り試す処理 | `src/engine/index.js` の `parseLlmJson()` |
| 用紙の言葉を台詞から削る処理 | 同 `sanitizeSay()` |
| ブラウザ内AIがE2Bを指していること | `src/llm.js` の `ON_DEVICE_MODEL_PATH` |
| 学習データの候補 | `playlogs/`(7ファイル、2026-08-13)、`src/chronicle.js` |
