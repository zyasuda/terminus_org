...
function adoptProposal(category, proposal, data = proposalData(category, proposal, contextFor(category))) {
  ...
  const node = nodeFor();
  let record;
  if (category === "secrets") { const secret = { ...data, id:secretId(node), trigger:"" }; (node.secrets ||= []).push(secret); proposal.recordId = secret.id; }
