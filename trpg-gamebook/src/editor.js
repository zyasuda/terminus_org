...
function aiPanel(node) {
  ...
  return `<section class="ai-panel"><div class="ai-heading"><h2>シナリオ補完</h2><button data-ai-settings type="button">Gemini設定</button></div><div class="ai-tabs" role="tablist">${tabs}</div>${message}<div class="conversation">${talk.map((item, talkIndex) => `<div class="talk ${item.role}" data-talk="${talkIndex}"><b>${item.role === "user" ? "作者" : "AI"}</b>${item.status ? `<p class="hint">${esc(item.status)}</p>` : ""}${item.thinking ? `<p class="hint">考えています</p>` : ""}<p>${esc(item.text)}</p>${item.role === "ai" ? cards(category, item) : ""}</div>`).join("")}</div>${existing}</section>`;
}
