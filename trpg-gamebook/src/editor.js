...
const save = () => { localStorage.setItem("gamebook:draft", JSON.stringify(chapter)); localStorage.setItem(TALKS_KEY, JSON.stringify([...talks])); $("draft").textContent = "下書きを編集中"; };
function loadTalks() {
  const stored = localStorage.getItem(TALKS_KEY);
  if (!stored) return;
  try {
    for (const [key, talk] of JSON.parse(stored)) if (Array.isArray(talk)) talks.set(key, talk);
  } catch {
    localStorage.removeItem(TALKS_KEY);
  }
}
...
function requiresSecretIds(requires) { return [...(requires?.secretsAll || []), ...(requires?.secretsAny || [])]; }
function chainLines(node, reveals = {}) {
  const roots = (node.secrets || []).filter(secret => !secret.requires && (secret.trigger || (secret.aliases || []).length));
  const line = (depth, text, count) => ({ depth, text, count });
  const secretCount = secret => reveals[secret.id] || 0;
  const walk = (secret, depth, path) => {
    if (depth >= 5 || path.has(secret.id)) return [];
    const nextPath = new Set(path).add(secret.id), lines = [];
    for (const loot of node.loot || []) if (loot.requires === secret.id) lines.push(line(depth + 1, `└ 調べると ${loot.name} を手に入れる`));
    for (const child of node.secrets || []) if (requiresSecretIds(child.requires).includes(secret.id) && !nextPath.has(child.id)) {
      lines.push(line(depth + 1, `└ ${child.entity} が現れる`, secretCount(child)));
      lines.push(...walk(child, depth + 1, nextPath));
    }
    for (const encounter of node.encounters || []) {
      const labels = new Set([secret.entity, ...(secret.aliases || [])]);
      if ((encounter.requiredElements || []).some(label => labels.has(label))) {
        lines.push(line(depth + 1, `└ ${encounter.enemy?.name || encounter.monsterName || "名前のない敵"} が現れる`));
        const revealed = (node.secrets || []).find(item => item.id === encounter.enemy?.revealOnDefeat);
        if (revealed && !nextPath.has(revealed.id)) {
          lines.push(line(depth + 2, `└ 倒すと ${revealed.entity} が現れる`, secretCount(revealed)));
          lines.push(...walk(revealed, depth + 2, nextPath));
        }
      }
    }
    for (const exit of node.exits || []) if (requiresSecretIds(exit.requires).includes(secret.id)) {
      const destination = (exit.match || [])[0];
      if (destination) lines.push(line(depth + 1, `└ ${destination}${destination.endsWith("へ") ? "進む" : destination.endsWith("進む") ? "" : "へ進む"} が選べるようになる`));
    }
    return lines;
  };
  return roots.flatMap(secret => [line(0, `${secret.entity} を調べる`, secretCount(secret)), ...walk(secret, 0, new Set())]);
}
function chainPanel(node) {
  const lines = chainLines(node, latestInspection?.play?.reveals);
  if (!lines.length) return `<section id="chain" hidden></section>`;
  return `<section id="chain"><h2>この場面の流れ</h2>${lines.map(({ depth, text, count }) => `<div class="chain-line" style="--chain-depth:${depth}">${esc(text)}${count === undefined ? "" : `<span class="chain-count ${count === 0 ? "zero" : ""}">${count}回</span>`}</div>`).join("")}</section>`;
}
function nodeEditor(node) {
  // 作ったものは畳まない。折りたたむと、作者は自分の成果が消えたと感じる
  return `${aiPanel(node)}${chainPanel(node)}<section id="author-form"><h2>${esc(node.name || "場面")}</h2>${field("場面名", "name", node.name)}${text("導入文", "brief", node.brief)}${text("進めないときの文", "blockedText", node.blockedText)}
...
function secretEditor(node, index) {
  ...
  const required = secret.requires?.secretsAll || [];
  const requiredOptions = (node.secrets || []).filter(other => other.id !== secret.id).map(other => `<option value="${esc(other.id)}" ${required.includes(other.id) ? "selected" : ""}>${esc(other.entity)}</option>`).join("");
  const loot = pendingProposal?.category === "secrets" ? pendingProposal.loot || "" : (node.loot || []).find(item => item.requires === secret.id)?.name || "";
  // 呼び名も trigger も無い秘密は pickExamineSecret が拾わない。作者に黙って作らせない
  const unreachable = !secret.trigger && !(secret.aliases || []).length;
  return `<h2>調べられるもの</h2>${field("要素名", "entity", secret.entity)}${field("呼び名（読点区切り）", "aliases", (secret.aliases || []).join("、"))}${unreachable ? `<p class="hint error">呼び名が空のあいだ、これは調べられません。</p>` : ""}${text("調べたときに分かること", "text", secret.text)}${text("見た目の説明", "surface", secret.surface)}${field("調べにくさ", "dc", secret.dc ?? 8, "number")}<label>調べる前に必要な発見<select multiple data-field="secretsAll">${requiredOptions}</select></label>${field("調べると手に入る物", "loot", loot)}<p class="hint">${pendingFor("secrets") ? "採用するまで章データには書き込みません。" : "IDと既存の trigger は保持します。"}</p><div class="actions">${pendingFor("secrets") ? `<button data-ai-adopt-pending type="button">採用</button>` : ""}<button data-back type="button">場面へ戻る</button></div>`;
}
...
function adoptProposal(category, proposal, data = proposalData(category, proposal, contextFor(category))) {
  ...
  const node = nodeFor();
  let record;
  if (category === "secrets") { const secret = { ...data, id:secretId(node), trigger:"" }; (node.secrets ||= []).push(secret); const loot = pendingProposal?.category === "secrets" ? pendingProposal.loot?.trim() : ""; if (loot) (node.loot ||= []).push({ name:loot, requires:secret.id }); proposal.recordId = secret.id; }
  else { record = applyProposal(category, proposal, node, { ...contextFor(category), decisions:nodes().map(([, item]) => item.decision).filter(Boolean) }, data); if (!record) return false; proposal.recordId = record.id; }
  proposal.adopted = true; const after = inspect(chapter); inspectProposal(proposal, before, after); save();
...
$('editor').addEventListener('click', event => { const target = event.target.closest('button'); if (!target) return; if (target.dataset.aiCategory) { selected = { ...selected, kind:'node', aiCategory:target.dataset.aiCategory }; renderEditor(); return; } const category = categoryFor(), talkIndex = Number(target.closest('[data-talk]')?.dataset.talk), proposal = talkFor(category)[talkIndex]?.proposals?.[Number(target.dataset.aiAdopt ?? target.dataset.aiEdit ?? target.dataset.aiDrop ?? target.dataset.aiUndo)]; if (target.dataset.aiAdopt !== undefined) { if (adoptProposal(category, proposal)) renderEditor(); } else if (target.dataset.aiEdit !== undefined) editProposal(category, proposal); else if (target.dataset.aiDrop !== undefined) { talkFor(category)[talkIndex].proposals.splice(Number(target.dataset.aiDrop), 1); save(); renderEditor(); } else if (target.dataset.aiUndo !== undefined) { const node = nodeFor(); if (category === 'decision') node.decision = null; else node[category] = (node[category] || []).filter(item => item.id !== proposal.recordId); proposal.adopted = false; delete proposal.recordId; save(); paintInspection(); renderEditor(); } else if (target.hasAttribute('data-ai-adopt-pending')) { if (adoptProposal(pendingProposal.category, pendingProposal.proposal, pendingProposal.data)) { pendingProposal = null; selected.kind = 'node'; renderEditor(); } } else if (target.dataset.select) { const [kind, index] = target.dataset.select.split(':'); selected = { ...selected, kind, ...(index !== undefined ? { index:Number(index) } : {}) }; renderEditor(); } else if (target.dataset.add) add(target.dataset.add); else if (target.dataset.remove) remove(target.dataset.remove); else if (target.hasAttribute('data-back')) { pendingProposal = null; selected.kind = 'node'; renderEditor(); } else if (target.dataset.suggest !== undefined) { const item = pendingProposal?.data || nodeFor().decision; item.choices[Number(target.dataset.suggest)].input = target.dataset.value; if (!pendingProposal) touch(); renderEditor(); } });
$("editor").addEventListener("submit", async event => { if (!event.target.matches("[data-ai-form]")) return; event.preventDefault(); const userText = new FormData(event.target).get("message")?.trim(); if (!userText) return; const category = categoryFor(), node = nodeFor(), talk = talkFor(category), reply = { role:"ai", text:"", proposals:[] }, model = getModel(); setTalk(category, [...talk, { role:"user", text:userText }, reply]); save(); renderEditor(); try { const result = await ask({ chapter, scene:node, category, context:contextFor(category, node), history:talk, userText, onStatus:status => { reply.status = status; renderEditor(); }, onThought:() => { reply.thinking = true; renderEditor(); }, onChunk:chunk => { reply.thinking = false; reply.text += chunk; renderEditor(); } }); reply.thinking = false; reply.text = result.reply; reply.proposals = result.proposals; save(); renderEditor(); } catch (error) { reply.thinking = false; reply.text = error.status === 404 ? "このモデルは今使えません。別のモデルを選んでください" : "つながりませんでした。もう一度試すか、下の入力欄で直接書けます"; if (error.status === 404) { openSettings(); loadModels(model); } save(); renderEditor(); } });
