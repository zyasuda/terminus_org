...
import { ask, parseReply } from "../src/ai.js";
...
console.log("ok 3 - 壊れたJSONでも返事を失わない");
{
  ...
  localStorage.setItem("gamebook:geminiKey", "test"); localStorage.setItem("gamebook:geminiModel", "gemini-3.5-flash-lite");
  ...
  const chunks = [], thoughts = [];
  const proposal = (await ask({ chapter, scene:chapter.scenes[0], userText:"油の匂いがある", onChunk:chunk => chunks.push(chunk), onThought:() => thoughts.push(true), transport:async (url, options) => { assert.equal(url, "https://generativelanguage.googleapis.com/v1/interactions"); const body = JSON.parse(options.body); assert.equal(body.model, "gemini-3.5-flash-lite"); assert.equal(body.stream, true); assert.equal(body.store, false); assert.equal(options.headers["x-goog-api-key"], "test"); return response; } })).proposals[0];
  assert.equal(thoughts.length, 1); assert.equal(chunks.join(""), "木柵の違和感が効いています。\n");
...
console.log("ok 4 - SSEの思考と本文を解釈し、採用後も完走できる");
