...
const addCompanion = (t, who = "gareth") => {
  ...
  t = String(t).replace(/」\s*[、,]?\s*「/g, " ").replace(/[「」]/g, "").replace(/[、,]\s*$/, "").trim();
  if (!t) return;
...
function maybeCompanion(r, addressed) {
  ...
  const who = normalizeWho(r.companion.who, "gareth");
...
function revealFlavor(secret) {
  ...
  callGmApi({
    ...
    if (r.say) addCompanion(String(r.say).slice(0, 80), normalizeWho(r.who, "lydia"));
  }).catch(() => {});
}
