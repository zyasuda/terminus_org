...
function configuredPath(value, fallback) {
  return value ? path.resolve(__dirname, value) : fallback;
}

const PROJECT_ROOT = path.resolve(__dirname, "..");
const MOCK2_DIR = configuredPath(process.env.MOCK2_DIR, path.join(PROJECT_ROOT, "trpg-gm-mock2"));
// 別環境では .env の MOCKDOCS_DIR を優先する。未指定時は一般的なDownloads配下を試す。
const MOCKDOCS_DIR = configuredPath(
  process.env.MOCKDOCS_DIR,
  path.join(os.homedir(), "Downloads", "BORG", "TRPG", "MockDocs")
);
// MVP.md 3章: 入力 = CAMPAIGN_01.md + GDD/AI_DESIGNのコンテキスト
const CONTEXT_FILES = ["CAMPAIGN_01.md", "AI_DESIGN.md"];
const OUTPUT_DIR = configuredPath(process.env.TAS_OUTPUT_DIR, path.join(__dirname, "output"));
const CAMPAIGN_OUTPUT_DIR = configuredPath(
  process.env.CAMPAIGN_OUTPUT_DIR,
  path.join(MOCK2_DIR, "public", "data")
);
// 画像の集約先はmock2のimagesフォルダ。下書きには /images/ファイル名 のパスだけを保持する
const IMAGES_DIR = configuredPath(process.env.MOCK_IMAGES_DIR, path.join(MOCK2_DIR, "images"));
const CAMPAIGN_CATALOG_PATH = path.join(CAMPAIGN_OUTPUT_DIR, "campaigns.json");

function readCampaignCatalog() {
  try {
    const value = JSON.parse(fs.readFileSync(CAMPAIGN_CATALOG_PATH, "utf-8"));
    return {
      version: Number(value.version) || 1,
      defaultCampaign: value.defaultCampaign || null,
      campaigns: Array.isArray(value.campaigns) ? value.campaigns : []
    };
  } catch (e) {
    return { version: 1, defaultCampaign: null, campaigns: [] };
  }
}

function writeCampaignCatalog(catalog) {
  const tempPath = `${CAMPAIGN_CATALOG_PATH}.tmp`;
  fs.writeFileSync(tempPath, JSON.stringify(catalog, null, 2) + "\n", "utf-8");
  fs.renameSync(tempPath, CAMPAIGN_CATALOG_PATH);
}

function updateCampaignCatalog(payload, campaignId, chapterId, chapterFile) {
  const catalog = readCampaignCatalog();
  const campaignPath = path.posix.join("campaigns", campaignId, "campaign.json");
  const chapterPath = path.posix.join("campaigns", campaignId, chapterFile);
  const entry = catalog.campaigns.find(item => item.id === campaignId) || {};
  const chapters = Array.isArray(entry.chapters) ? [...entry.chapters] : [];
  const nextChapter = { id: chapterId, title: payload.chapter.title || chapterId, file: chapterPath };
  const chapterIndex = chapters.findIndex(item => item.id === chapterId);
  if (chapterIndex >= 0) chapters[chapterIndex] = nextChapter;
  else chapters.push(nextChapter);
  const nextEntry = {
    ...entry,
    id: campaignId,
    title: payload.campaign.meta?.title || payload.campaign.title || campaignId,
    campaign: campaignPath,
    defaultChapter: entry.defaultChapter || chapterId,
    chapters
  };
  const campaignIndex = catalog.campaigns.findIndex(item => item.id === campaignId);
  if (campaignIndex >= 0) catalog.campaigns[campaignIndex] = nextEntry;
  else catalog.campaigns.push(nextEntry);
  if (!catalog.defaultCampaign) catalog.defaultCampaign = campaignId;
  catalog.version = Number(catalog.version) || 1;
  writeCampaignCatalog(catalog);
  return { campaignPath, chapterPath };
}
...
/* JSON中の画像ファイル名参照(data:や旧assets/パスは除く)を再帰的に集める。
   キー名を列挙する方式は将来のスキーマ変更に追従できないため、値の形で汎用的に判定する */
function collectImageRefs(value, out) {
  if (typeof value === "string") {
    if (/\.(png|jpe?g|gif|webp)$/i.test(value) && !value.startsWith("data:") && !value.includes("/")) out.push(value);
  } else if (Array.isArray(value)) {
    value.forEach(v => collectImageRefs(v, out));
  } else if (value && typeof value === "object") {
    Object.values(value).forEach(v => collectImageRefs(v, out));
  }
}

/* mock2の読み込み時検証(trpg-gm-mock2/src/scenario.js validate)と同一ルール。
   ここでNGを返せば、mock2側で起動時エラーになるデータを書き込まない。
   ID一意性・エンティティ参照切れ・画像参照はRustの共有データモデル(rust/trpg-data-model)の
   validate()と同じ観点をJS側にも実装し、書き込み前に必ず検査する(出力前検証の必須化) */
function validateCampaignData(campaign, chapter) {
  ...
  if (!Array.isArray(campaign.companions) || campaign.companions.length === 0) {
    ...
    campaign.companions.forEach((c, i) => {
      ...
      if (c.id && !/^member_[1-9][0-9]*$/.test(c.id)) errs.push(`companions[${i}].id "${c.id}" は member_1 形式である必要があります`);
      if (c.id && companionIds.has(c.id)) errs.push(`companions の id が重複: ${c.id}`);
      if (c.id) companionIds.add(c.id);
    });
  }
  const entityNames = new Set((campaign.entities || []).map(e => (typeof e === "string" ? e : e.ja)).filter(Boolean));
  (chapter.flagsOut || []).forEach(flag => {
    if (!campaign.flags || !(flag in campaign.flags)) errs.push(`flagsOut "${flag}" が campaign.flags に未宣言`);
  });
  ...
  if (!Array.isArray(chapter.scenes) || chapter.scenes.length === 0) {
    ...
    chapter.scenes.forEach((sc, i) => {
      ...
      if (sc.id !== undefined && sceneIds.has(sc.id)) errs.push(`scene id が重複: ${sc.id}`);
      if (sc.id !== undefined) sceneIds.add(sc.id);
      if (sc.enemy?.name && entityNames.size && !entityNames.has(sc.enemy.name)) {
        errs.push(`${label}: 敵 "${sc.enemy.name}" が entities にありません`);
      }
      (sc.loot || []).forEach(raw => {
        const name = typeof raw === "string" ? raw : raw?.name;
        if (name && entityNames.size && ![...entityNames].some(n => name.startsWith(n) || n.startsWith(name))) {
          errs.push(`${label}: loot "${name}" が entities にありません`);
        }
      });
  ...
  // 画像参照切れの検査(mock2のimagesフォルダに実ファイルがあるか)
  const imageRefs = [];
  collectImageRefs(campaign, imageRefs);
  collectImageRefs(chapter, imageRefs);
  [...new Set(imageRefs)].forEach(name => {
    if (!fs.existsSync(path.join(IMAGES_DIR, name))) errs.push(`画像参照が見つかりません: ${name}(${IMAGES_DIR}に実ファイルが必要)`);
  });
  return errs;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && req.url === "/api/context") {
    ...
    // 正本はcampaigns.jsonが指定するキャンペーン別ネスト構造。
    // TAS内部の既存参照キーへ、既定キャンペーンの内容を別名で提供する。
    const catalog = readCampaignCatalog();
    const campaignEntry = catalog.campaigns.find(item => item.id === catalog.defaultCampaign) || catalog.campaigns[0];
    if (campaignEntry?.campaign) {
      try { dataFiles["campaign.json"] = fs.readFileSync(path.join(dataDir, campaignEntry.campaign), "utf-8"); } catch (e) {}
    }
    const defaultChapter = campaignEntry?.defaultChapter || campaignEntry?.chapters?.[0]?.id;
    const chapterEntry = campaignEntry?.chapters?.find(item => item.id === defaultChapter) || campaignEntry?.chapters?.[0];
    if (chapterEntry?.file) {
      try { dataFiles[`${defaultChapter}.json`] = fs.readFileSync(path.join(dataDir, chapterEntry.file), "utf-8"); } catch (e) {}
    }
  ...
  if (req.method === "POST" && req.url === "/api/export-campaign") {
    try {
      ...
      const campaignId = String(payload.campaignId || payload.campaign.meta?.id || "").trim();
      const chapterId = String(payload.chapterId || `chapter_${String(payload.chapter.id || 1).padStart(2, "0")}`).trim();
      if (!/^[a-z0-9][a-z0-9_-]*$/.test(campaignId)) {
        return json(400, { error: { message: "campaignId は小文字英数字・_・-で指定してください" } });
      }
      ...
      const campaignDir = path.join(CAMPAIGN_OUTPUT_DIR, "campaigns", campaignId);
      fs.mkdirSync(campaignDir, { recursive: true });
      const campaignPath = path.join(campaignDir, "campaign.json");
      const chapterPath = path.join(campaignDir, chapterFile);
      ...
      const relative = updateCampaignCatalog(payload, campaignId, chapterId, chapterFile);
      return json(200, {
        saved: [path.join(CAMPAIGN_OUTPUT_DIR, relative.campaignPath), path.join(CAMPAIGN_OUTPUT_DIR, relative.chapterPath), CAMPAIGN_CATALOG_PATH],
        dir: CAMPAIGN_OUTPUT_DIR,
        campaignId,
        chapterId,
        canonical: relative
      });
...
server.listen(PORT, "127.0.0.1", () => {
  ...
  console.log(`mock2データ出力先: ${CAMPAIGN_OUTPUT_DIR}`);
});
