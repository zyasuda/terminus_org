...
renderWorld=function(){
  ...
  if(card){
    const cast=[{id:'gm',name:castName('gm','GM')},{id:'gareth',name:castName('gareth','ガレス')},...Array.from({length:extraCompanions},(_,i)=>({id:`member_${i+2}`,name:castName(`member_${i+2}`,`メンバー${i+2}`)})),...npcList().map(x=>({id:x.id,name:castName(x.id,x.name)}))];
