...
const castAttribute=(id)=>{const value=castAttributes[id]||{};const companionIndex=id==="gareth"?0:/^member_(\d+)$/.test(id)?Number(RegExp.$1)-1:-1;let source={};try{if(companionIndex>=0)source=JSON.parse(context?.dataFiles?.["campaign.json"]||"{}").companions?.[companionIndex]||{}}catch(e){}const raw={...source,...value};const addressTerm=String(raw.addressTerm||raw.addressing||"");const speechFrequency=["none","low","standard","high"].includes(raw.speechFrequency)?raw.speechFrequency:"standard";const result={gender:raw.gender||"unspecified",firstPerson:String(raw.firstPerson||""),addressTerm,speechRules:String(raw.speechRules||""),speechFrequency};for(const key of ["quirks","battleMutters","battleEnd","idleLine"])if(Object.prototype.hasOwnProperty.call(raw,key))result[key]=raw[key];return result;};
