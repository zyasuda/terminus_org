...
- 1〜2文字の語は、別の要素の説明文や他のトリガー語句に紛れ込んで誤爆する。
...
# 出力スキーマ
{"nodes":[{"node":{"type":"opening|scene|ending","index":0},"exits":[{"id":"既存exit id","match":["トリガー語句"]}],"discoveries":[{"id":"既存discovery id","aliases":["別名"]}]}]}
...
function parseMatchWordProposals(text){
  ...
  if(!Array.isArray(data.nodes))throw new Error('トリガー語句候補の nodes 配列がありません');
...
function applyMatchWordProposal(node,kind,id){
  ...
  sceneOverrides[key]=override;saveWorkspaceDraft(true);renderScenes();renderAll();setStatus('トリガー語句候補を採用しました');
}
function applyAllMatchWordProposals(){
  ...
  if(!count)setStatus('採用できるトリガー語句候補がありません');
}
function renderMatchWordCandidate(node,kind,current,proposal){
  const values=kind==='exit'?proposal.match:proposal.aliases;const label=kind==='exit'?`出口 ${proposal.id}`:`調査対象 ${current.label}`;
...
function renderMatchWords(){
  ...
  return `<div class="draft-area"><h2>トリガー語句候補</h2><p class="hint">イントロ・全シーン・アウトロの出口トリガー語句と調査対象の別名を、AIが1回で候補化します。生成しただけでは下書きに反映しません。</p><div class="bottom"><button id="btnGenerateMatchWords">章全体の候補を生成</button><button class="sub" id="btnApplyAllMatchWords" ${sections?'':'disabled'}>候補を一括採用</button></div>${matchWordProposalError?`<p class="validation error">${escapeHtml(matchWordProposalError)}</p>`:''}${sections||'<p class="match-word-empty">候補はまだありません。</p>'}</div>`
}
function bindMatchWords(){
  const generate=$('btnGenerateMatchWords');if(generate)generate.onclick=async()=>{generate.disabled=true;matchWordProposalError='';setStatus('章全体のトリガー語句候補を生成中…');try{const text=await callLLM('あなたはTRPGのトリガー語句を安全に設計する共同制作者です。出力は指定スキーマのJSONオブジェクト1個のみ。', [{role:'user',content:matchWordGenerationPrompt()}],8000);const next=parseMatchWordProposals(text);matchWordProposals=next;renderTab();setStatus('トリガー語句候補を生成しました。現在値と比較して採用してください');}catch(error){matchWordProposalError=`トリガー語句候補の生成に失敗しました: ${error.message}`;renderTab();setStatus(matchWordProposalError)}finally{generate.disabled=false}};
...
renderTabs=function(){baseRenderTabsForMatchWords();const tabs=$('layerTabs');if(tabs&&!tabs.querySelector('[data-tab="matchWords"]'))tabs.insertAdjacentHTML('beforeend',`<button class="tab ${activeTab==='matchWords'?'active':''}" data-tab="matchWords">トリガー語句候補</button>`)};
...
function outputMatchWordExits(payload){
  ...
  [['opening','intro'],['ending','ending']].forEach(([type,key])=>{
    ...
    const exits=override.exits.map(normalizeExit).filter(exit=>exit.match.length).map((exit,index)=>{const to=outputExitTarget(exit.to);const requires=outputExitRequires(exit.requires,token=>{const target=(node.discoveries||[]).map((value,targetIndex)=>normalizeDiscoveryFor(node,value,targetIndex)).find(target=>target.id===token||target.label===token);return target?.id},node);return {id:exit.id||`exit_${index+1}`,match:exit.match,...(to===null?{to:null}:{to}),...(requires&&Object.keys(requires).length?{requires}:{}),...(exit.removeItems?.length?{removeItems:exit.removeItems}:{}),...(exit.addItems?.length?{addItems:exit.addItems}:{}),...(exit.npcSay?{npcSay:exit.npcSay}:{}),...(exit.blockedText?{blockedText:exit.blockedText}:{}),...(exit.text?{text:exit.text}:{})}});
