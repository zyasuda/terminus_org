...
mockCampaignPayload=function(){
  ...
  [['opening','intro'],['ending','ending']].forEach(([type,key])=>{
    ...
    const exits=override.exits.map(normalizeExit).filter(exit=>exit.match.length).map((exit,index)=>{const to=exit.to===null?null:exit.to==='end'?'end':Number.isFinite(Number(exit.to))?Number(exit.to):exit.to||null;const requires=exit.requires?.text?exitRequires(exit.requires.text,token=>{const target=(node.discoveries||[]).map((value,targetIndex)=>normalizeDiscoveryFor(node,value,targetIndex)).find(target=>target.id===token||target.label===token);return target?.id},node):exit.requires;return {id:exit.id||`exit_${index+1}`,match:exit.match,...(to===null?{to:null}:{to}),...(requires&&Object.keys(requires).length?{requires}:{}),...(exit.removeItems?.length?{removeItems:exit.removeItems}:{}),...(exit.addItems?.length?{addItems:exit.addItems}:{}),...(exit.npcSay?{npcSay:exit.npcSay}:{}),...(exit.blockedText?{blockedText:exit.blockedText}:{}),...(exit.text?{text:exit.text}:{})}});
