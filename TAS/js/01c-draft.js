...
function gamePayloadToWorkspaceDraft(raw){
  ...
  const sceneBackgrounds={};scenes.forEach((s,i)=>{if(s&&s.img)sceneBackgrounds[`ch1:scene:${s.id??i+1}`]=importedAssetRef(s.img)});
  ...
  scenes.forEach((s,i)=>{const key=`ch1:scene:${s?.id??i+1}`;const image=sceneBackgrounds[key];const sky=importedAssetRef(s?.parallax?.sky);if(image||sky)sceneOverrides[key]={...(image?{img:image}:{}),...(sky?{parallaxSky:sky}:{})};});
