...
function structureSceneLabel(node,index){return node.type==='scene'?`シーン${node.id??index+1}`:node.type==='opening'?'イントロ':node.type==='ending'?'アウトロ':'インターミッション'}
