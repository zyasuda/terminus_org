  ...
  while (rooms.size < scenes.length) {
    ...
    for (const link of candidates) {
      ...
      for (const direction of directions) {
        ...
        if (d.x) { child.x = d.x > 0 ? parent.x + parent.w + gap : parent.x - gap - child.w; child.y = parent.y + randomInt(rng, [-child.h + 1, parent.h - 1]); }
        else { child.y = d.y > 0 ? parent.y + parent.h + gap : parent.y - gap - child.h; child.x = parent.x + randomInt(rng, [-child.w + 1, parent.w - 1]); }
        if ([...rooms.values()].some(room => tooClose(child, room))) continue;
        const candidateRooms = new Map(rooms).set(childId, child);
        const path = routeBetween(portal(parent, d.name), portal(child, opposite(d.name)), candidateRooms, corridors, rng);
        ...
        usedPorts.get(parentId).add(d.name); usedPorts.get(childId).add(opposite(d.name));
        placed = true; break;
      }
      if (placed) break;
    }
