// 遠征戦闘でプレイ後に調整する値の単独正本です。
// d20判定・経路探索・レンダラー内部の計算は、ここには置きません。
export const EXPEDITION_BATTLE_CONFIG = {
  board: {
    /** 通常戦の細長い通路盤面。数値を変えると開始位置と敵との距離感が変わる。単位: マス。 */
    corridor: {
      /** 通路の横幅。大きいほど接敵までのターン数が増える。単位: マス。 */
      width: 7,
      /** 通路の縦幅。大きいほど横並びの位置取りの余地が増える。単位: マス。 */
      height: 3,
      /** 通常戦で味方2人を置く候補。位置を変えると初手の選択肢が変わる。単位: グリッド座標。 */
      partySlots: [{ x: 0, y: 0 }, { x: 0, y: 2 }],
      /** 通常戦の敵開始位置(1体目)。味方から遠ざけるほど接敵までが長くなる。単位: グリッド座標。 */
      enemyStart: { x: 6, y: 0 },
      /** 通常戦の敵開始位置(2体目)。通路の縦幅いっぱいに上下へ分けて置く。単位: グリッド座標。 */
      enemyStart2: { x: 6, y: 2 },
    },
    /** 守護者戦の正方形盤面。通常戦とは別に広さと初期配置を調整できる。単位: マス。 */
    guardian: {
      /** 守護者盤面の横幅。大きいほど回り込みの余地が増える。単位: マス。 */
      width: 8,
      /** 守護者盤面の縦幅。大きいほど回り込みの余地が増える。単位: マス。 */
      height: 8,
      /** 守護者戦で味方2人を置く候補。位置を変えると初手の隊列が変わる。単位: グリッド座標。 */
      partySlots: [{ x: 1, y: 3 }, { x: 1, y: 4 }],
      /** 守護者の開始位置。味方に近づけるほど初期の緊張感が増す。単位: グリッド座標。 */
      enemyStart: { x: 6, y: 3 },
    },
    /** 三叉路での固定遭遇盤面。枝以外を盤外として、地図の三方向を戦闘にも残す。 */
    junction: {
      /** 三叉路の歩行可能マス。'.'=床、'#'=盤外。各枝を3マス幅にして位置取りの余地を残す。 */
      rows: ["##...##", "##...##", ".....##", ".....##", ".....##", "##...##", "##...##"],
      /** 三叉路へ入った味方2人の開始候補。幅3の西入口内で上下に並べる。単位: グリッド座標。 */
      partySlots: [{ x: 0, y: 2 }, { x: 0, y: 4 }],
      /** 三叉路遭遇の敵開始位置。北の枝に置く。単位: グリッド座標。 */
      enemyStart: { x: 3, y: 0 },
    },
    obstacles: {
      /** 障害物の最少数。増やすほど毎戦の地形差が強くなる。単位: 個。 */
      min: 1,
      /** 障害物の最多数。増やすほど移動ルートが複雑になる。単位: 個。 */
      max: 6,
      /** 高さ0.5のブロックへ隣接して置く踏み台の高さ。高くすると登れる段差が大きくなる。単位: タイル高。 */
      rampHeight: 0.25,
      /** 高さ0.5のブロックごとに踏み台を置く確率。高くすると登坂経路が増える。単位: 確率(0〜1)。 */
      rampChance: 1.0,
    },
  },
  movement: {
    /** 通常キャラが隣接マス間で上り下りできる段差の上限。低くすると踏み台が必要になる。単位: タイル高。 */
    maxStep: 0.25,
  },
  units: {
    /** 主人公の基礎値。装備の補正はatk/hpへ加算される。 */
    hero: {
      /** HUDに表示する主人公名。 */ name: "あなた",
      /** 表示に使うGLBモデルID。厚みのあるアクリル板スタンディ(assets/standee/sources.json)。 */ modelId: "gareth-standee",
      /** 装備前の攻撃力。高いほど通常攻撃のダメージが増える。単位: ダメージ。 */ atk: 3,
      /** 装備前の最大HP。高いほど倒れにくい。単位: HP。 */ hp: 16,
      /** 狙われた時の避けやすさ。低いほど敵の攻撃が当たりやすい。d20の判定基準。単位: 難易度値。 */ defenseDc: 10,
      /** 行動順と移動力に使う敏捷。高いほど先手と移動距離を得やすい。単位: 敏捷値。 */ agility: 7,
      /** 3D上のモデルの見かけの高さ。ゲーム上の当たり判定は変わらない。単位: タイル高。 */ height: 1.6,
      /** 高い障害物を登れるか。falseならmovement.maxStepを超える段差を上り下りできない。 */ canClimb: false,
    },
    /** リディアの基礎値。遠隔攻撃の演出と行動順に使う。 */
    mage: {
      /** HUDに表示する相棒名。 */ name: "リディア",
      /** 攻撃が遠隔か。trueなら隣接ではなく射線(resolveRanged)の通る敵を狙う。 */ ranged: true,
      /** 表示に使うGLBモデルID。厚みのあるアクリル板スタンディ(assets/standee/sources.json)。 */ modelId: "lydia-standee",
      /** 装備前の攻撃力。高いほど魔法のダメージが増える。単位: ダメージ。 */ atk: 2,
      /** 装備前の最大HP。高いほど倒れにくい。単位: HP。 */ hp: 12,
      /** 狙われた時の避けやすさ。低いほど敵の攻撃が当たりやすい。d20の判定基準。単位: 難易度値。 */ defenseDc: 10,
      /** 行動順と移動力に使う敏捷。高いほど先手と移動距離を得やすい。単位: 敏捷値。 */ agility: 5,
      /** 3D上のモデルの見かけの高さ。ゲーム上の当たり判定は変わらない。単位: タイル高。 */ height: 1.6,
      /** 高い障害物を登れるか。falseならmovement.maxStepを超える段差を上り下りできない。 */ canClimb: false,
    },
    /** 三叉路など、通路戦以外の通常遭遇(1体)の基礎値。 */
    enemy: {
      /** HUDとログに表示する敵名。 */ name: "坑道の獣",
      /** 表示に使うGLBモデルID。厚みのあるアクリル板スタンディ(assets/standee/sources.json)。 */ modelId: "rust-eater-standee",
      /** 敵の攻撃力。高いほど主人公へのダメージが増える。単位: ダメージ。 */ atk: 2,
      /** 敵の最大HP。高いほど撃破までの攻撃回数が増える。単位: HP。 */ hp: 8,
      /** 狙われた時の避けやすさ。低いほど味方の攻撃が当たりやすい。d20の判定基準。単位: 難易度値。 */ defenseDc: 10,
      /** 行動順と移動力に使う敏捷。高いほど先手と移動距離を得やすい。単位: 敏捷値。 */ agility: 4,
      /** 3D上のモデルの見かけの高さ。ゲーム上の当たり判定は変わらない。単位: タイル高。 */ height: 0.9,
      /** 高い障害物を登れるか。坑道の獣は高所ブロックにも乗れる。 */ canClimb: true,
      /** HPがこの割合以下になったら、戦わずに離脱を試みる。0なら逃げない。単位: 比率(0〜1)。 */ fleeHpRatio: 0.3,
    },
    /** 通路戦(2体)の基礎値。2体分の合計攻撃回数・合計HPが1体の時より重くなりすぎないよう、
     * enemyより弱めにしてある。 */
    corridorEnemy: {
      /** HUDとログに表示する敵名。 */ name: "坑道の獣",
      /** 表示に使うGLBモデルID。厚みのあるアクリル板スタンディ(assets/standee/sources.json)。 */ modelId: "rust-eater-standee",
      /** 敵の攻撃力。高いほど主人公へのダメージが増える。単位: ダメージ。 */ atk: 1,
      /** 敵の最大HP。高いほど撃破までの攻撃回数が増える。単位: HP。 */ hp: 6,
      /** 狙われた時の避けやすさ。低いほど味方の攻撃が当たりやすい。d20の判定基準。単位: 難易度値。 */ defenseDc: 10,
      /** 行動順と移動力に使う敏捷。高いほど先手と移動距離を得やすい。単位: 敏捷値。 */ agility: 4,
      /** 3D上のモデルの見かけの高さ。ゲーム上の当たり判定は変わらない。単位: タイル高。 */ height: 0.9,
      /** 高い障害物を登れるか。坑道の獣は高所ブロックにも乗れる。 */ canClimb: true,
      /** HPがこの割合以下になったら、戦わずに離脱を試みる。0なら逃げない。単位: 比率(0〜1)。 */ fleeHpRatio: 0.3,
    },
    /** 宝箱守護者の基礎値。通常敵との差をここだけで調整する。 */
    guardian: {
      /** HUDとログに表示する敵名。 */ name: "宝箱守護者",
      /** 表示に使うGLBモデルID。厚みのあるアクリル板スタンディ(assets/standee/sources.json)。 */ modelId: "rust-eater-standee",
      /** 守護者の攻撃力。高いほど主人公へのダメージが増える。単位: ダメージ。 */ atk: 3,
      /** 守護者の最大HP。高いほど撃破までの攻撃回数が増える。単位: HP。 */ hp: 9,
      /** 狙われた時の避けやすさ。低いほど味方の攻撃が当たりやすい。d20の判定基準。単位: 難易度値。 */ defenseDc: 10,
      /** 行動順と移動力に使う敏捷。高いほど先手と移動距離を得やすい。単位: 敏捷値。 */ agility: 4,
      /** 3D上のモデルの見かけの高さ。ゲーム上の当たり判定は変わらない。単位: タイル高。 */ height: 0.9,
      /** 高い障害物を登れるか。守護者は高所ブロックにも乗れる。 */ canClimb: true,
      /** 守護者は宝箱を守る役目上、瀕死でも逃げない。単位: 比率(0〜1)。 */ fleeHpRatio: 0,
      /** 体表を暗くする色調(元の色に乗算)。通常の坑道の獣と見分けが付くよう黒っぽくしてある。単位: 16進カラー。 */ tint: 0x3a3a3a,
    },
  },
  companion: {
    /** リディアのHPがこの割合以下になったら、相棒指示に関わらず自ら入口側へ後退する。単位: 比率(0〜1)。 */
    lowHpRetreatRatio: 0.3,
  },
  encounter: {
    /** 遭遇時、不意打ちで敵が先手を取る確率。それ以外は通常どおりagility順。単位: 比率(0〜1)。 */
    enemyFirstChance: 0.2,
  },
  timing: {
    /** AIが行動を選ぶ前の見せる間。長いほどAIの判断を読み取りやすい。単位: ミリ秒。 */
    aiThinkMs: 650,
    /** 移動演出後に次の手番へ渡すまでの間。長いほど移動を認識しやすい。単位: ミリ秒。 */
    moveSettleMs: 800,
    /** 攻撃演出後に次の手番へ渡すまでの間。長いほど攻撃結果を読み取りやすい。単位: ミリ秒。 */
    attackSettleMs: 1000,
    /** 通常の手番切替に使う待ち時間。長いほど手番の変化を追いやすい。単位: ミリ秒。 */
    turnTransitionMs: 350,
    /** 攻撃時に対面カメラを保つ時間。長いほど攻撃の印象が強まる。単位: ミリ秒。 */
    attackCameraMs: 700,
  },
  presentation: {
    /** 遠征戦闘の周囲に背景壁を表示するか。falseにすると開放感が増す。単位: 真偽値。 */
    showBackdropWalls: true,
    /** カメラの見下ろし角。上げるほど見下ろしが強くなり、
     * 手前の障害物が奥の盤面・ユニットと重なりにくくなる。注視点と水平距離は変わらない。単位: 度。
     * 戦闘画面下部のスライダーで見た目を見ながら調整できる(この値は既定値)。
     * スタンディ(垂直なアクリル板)は見下ろすほど絵が短縮されて潰れるため、
     * 2026-08-25に45度から20度へ下げた。角度を下げた分、奥のユニットが手前と
     * 重なりやすくなる。重なりはcameraAzimuthOffsetDegと遮蔽フェードで外す。 */
    cameraElevationDeg: 20,
    /** heroの向きを画面奥にする水平角度から、さらにずらす角度。
     * 0にするとheroが向いている敵と画面上で正対し、hero自身が敵を隠しやすくなる。
     * 0以外にすると隠れにくくなる代わりに、方位角が45度の倍数から外れて
     * 盤面の格子が菱形にならず、アイソメトリックらしさが崩れる。
     * 手前の味方が対象を隠すときは遮蔽フェード(view3d.jsのsetUnitOccluded)も
     * 同じ症状に効くので、この値は0でも成り立つ可能性がある。単位: 度。 */
    cameraAzimuthOffsetDeg: 20,
    /* 戦闘中の既定のアングルは「ポケモンアングル」(作者の命名 2026-08-27)。
     * 手番の駒と、その駒が狙う敵の中間を注視点にし、駒の背中越しに敵を見る構図。
     * 向きは azimuthForFacing、注視点は pokemonAngle()(ExpeditionBattle.jsx)が決める。
     * ここにある cameraElevationDeg / cameraZoom / cameraAzimuthOffsetDeg の3つが
     * このアングルの既定値で、画面右上のコンパスをタップするとこの値へ戻る。
     * 攻撃の瞬間だけ対面カメラ(view3d.jsのsetCameraFocus)へ切り替わる。 */
    /** 戦闘盤面のズーム倍率。大きいほど盤面が画面いっぱいに近づく。
     * 画面下部のスライダーで 0.75〜3.0 の範囲を調整できる(この値は既定値)。 */
    cameraZoom: 2.5,
    /** ゆらゆら立ち上る塵を最初から出すか。空気感が増えるが、静止画では埃が乗って見える。
     * 画面下部の「塵」チェックボックスで切り替えられる(この値は既定値)。単位: 真偽値。 */
    dust: true,
    /** 盤面の外側(空)の色。暗いほど盤面が浮き、明るいほど背景と馴染む。
     * 画面下部の「背景」で調整できる(この値は既定値)。単位: 16進カラー。 */
    backgroundColor: "#061128",
    modelFacingOffset: {
      /** 味方GLBのローカル正面補正。値を変えると見た目だけがY軸回転する。単位: ラジアン。 */
      party: 0,
      /** 敵GLBのローカル正面補正。値を変えると見た目だけがY軸回転する。単位: ラジアン。 */
      enemy: 0,
    },
  },
};
