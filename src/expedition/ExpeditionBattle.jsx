...
const makeState = (guardian, equipment = {}, party = {}) => {
  ...
  const units = [
    { id: "hero", name: "あなた", side: "party", x: 1, y: 3, hp: Math.min(hero.hp, party.hero ?? hero.hp), maxHp: hero.hp, atk: hero.atk, agility: 7, height: 1.6, modelId: "gareth" },
    { id: "mage", name: "リディア", side: "party", x: 1, y: 4, hp: Math.min(mage.hp, party.mage ?? mage.hp), maxHp: mage.hp, atk: mage.atk, agility: 5, height: 1.6, modelId: "lydia" },
    { id: "enemy", name: guardian ? "宝箱守護者" : "坑道の獣", side: "enemy", x: 6, y: 3, hp: guardian ? 18 : 8, maxHp: guardian ? 18 : 8, atk: guardian ? 3 : 2, agility: 4, height: .9, modelId: "rust-eater" }
  ];
  return { grid: createGrid(rows), units, order: turnOrder(units).map(u => u.id), turn: 0, log: [guardian ? "守護者が宝箱を守っている。" : "坑道の獣が行く手を塞いだ。"] };
};
...
export default function ExpeditionBattle({ guardian, order, equipment = {}, party = {}, tonics = 0, onUseTonic, onFinish }) {
  ...
  const [command, setCommand] = useState(order), [moved, setMoved] = useState(false);
  ...
  const nextTurn = () => { setMoved(false); setState(s => { for (let i = 1; i <= s.order.length; i++) { const turn = (s.turn + i) % s.order.length; if (s.units.find(u => u.id === s.order[turn])?.hp > 0) return { ...s, turn }; } return s; }); };
  const damage = (attacker, target, ranged = false) => {
    ...
    if (!result.ok) return;
    scene.current?.setCameraFocus(attacker, target);
    setTimeout(() => scene.current?.setCameraFocus(null), 420);
  ...
  useEffect(() => {
    ...
    s.setPickHandler(data => {
      ...
      if (t && targets.some(x => x.id === t.id)) { damage(active, t); setTimeout(nextTurn, 500); }
  ...
  }, [state, active?.id, playerTurn, moved]);
  useEffect(() => {
  ...
  return <div style={S.page}><div ref={mount} style={S.canvas}/><div style={S.hud}>
    ...
    <div style={S.row}><span>相棒指示:</span>{[["attack","攻撃"],["guard","護衛"],["retreat","退却"]].map(([id,label]) => <button key={id} style={{...S.btn, ...(command === id ? S.active : {})}} onClick={() => setCommand(id)}>{label}</button>)}<button style={S.btn} onClick={() => scene.current?.rotate(1)}>視点</button>{playerTurn && <button style={S.btn} onClick={heroAdvance}>接近／攻撃</button>}{playerTurn && <button style={S.btn} onClick={nextTurn}>ターン終了</button>}{playerTurn && <button disabled={!tonics} style={S.btn} onClick={() => { if (onUseTonic?.()) setState(s => ({ ...s, units: s.units.map(u => u.id === "hero" ? { ...u, hp: Math.min(u.maxHp, u.hp + ITEMS.tonic.power) } : u), log: [...s.log, "回復薬を使った。"] })); }}>回復薬 ({tonics})</button>}</div>
